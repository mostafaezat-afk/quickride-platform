...
function UserEditProfile() {
  ...
  const updateUserProfile = async (data) => {
    ...
    try {
      ...
      showAlert('تم بنجاح', 'تم تحديث بياناتك الشخصية بنجاح', 'success');

      setTimeout(() => {
        navigation("/home");
      }, 5000)
    } catch (error) {
      showAlert('حدث خطأ', error.response?.data?.[0]?.msg || 'حدث خطأ أثناء التحديث', 'failure');
    ...
    <div className="w-full h-dvh flex flex-col justify-between p-4 pt-6" style={{ fontFamily: 'Cairo, sans-serif' }}>
      ...
      <div>
        <div className="flex gap-3">
          ...
          />
          <Heading title={"تعديل الملف الشخصي"} />
        </div>
