...
function ChatScreen() {
  ...
  const getUserDetails = async () => {
    try {
      ...
      if (currentUser !== response.data.user._id && currentUser !== response.data.captain._id) {
        Console.log("غير مصرح لك بعرض هذه المحادثة.");
        navigation(-1);
        return;
      }
      ...
      Console.log("هذه الرحلة غير موجودة.");
    }
  };
  ...
  if (userData) {
    return (
      <div className="flex flex-col h-dvh" style={{ fontFamily: 'Cairo, sans-serif' }}>
        {/* header */}
                ...
                return (
                  <span
                    ...
                    className={`${message.by == userType
                      ? "mr-auto rounded-bl-none bg-blue-600 text-white"
                      : "ml-auto rounded-br-none bg-white"
                      } rounded-xl mb-1 px-3 pt-2 pb-[3px] text-sm max-w-64 leading-4`}
                  >
                    {message.msg}
                    <div className="text-[10px] font-normal text-left opacity-60 mt-[1px]">{message.time}</div>
                  </span>
                );
              })}
          </div>
